...
export class Player {
    ...
    fire(isBurstShot = false) {
        ...
        if (this.fireCooldown <= 0 || isBurstShot) {
            ...
            // Missile add-on frequency is counted from accepted initial gun shots.
            // Burst rounds do not advance it, and every trigger emits at most one missile.
            if (!isBurstShot && this.hasMissile) {
                const gunShotsPerMissile = MAX_MISSILE_LEVEL + 1 - this.missileLevel;
                this.missileShotCounter++;
                if (this.missileShotCounter >= gunShotsPerMissile) {
                    this.missileShotCounter = 0;
