...
test('creating and updating one trimmed profile preserves every other slot', () => {
    ...
    assert.equal(created.totalXP, 100);
    assert.deepEqual(store.getSummaries(), [null, null, { slot: 2, name: 'Nova', level: 1 }, null, null]);
    ...
    assert.deepEqual(store.getProfile(2), {
        version: 1, slot: 2, name: 'Nova', level: 4, totalXP: 750, pendingLevelUps: 1,
        projectileUpgradeCount: 1, speedUpgradeCount: 1, levelShieldUpgradeCount: 1
    });
...
test('stored values are normalized, truncated, migrated, and stripped of temporary data', () => {
    ...
    assert.deepEqual(profiles[0], {
        version: 1, slot: 0, name: 'Pilot 0', level: 1, totalXP: 0, pendingLevelUps: 1,
